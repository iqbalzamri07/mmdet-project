...
def extract_keypoints_from_video(video_path, det_model, pose_model):
    ...
    return keypoints_array, keypoint_scores_array, (height, width)
...
def process_dataset(split="train"):
    ...
    for label in ACTION_LABELS:
        ...
        for video_file in video_files:
            ...
            try:
                keypoints, keypoint_scores, img_shape = extract_keypoints_from_video(
    ...
    # Save data in PoseDataset format
    output_file = os.path.join(OUTPUT_ROOT, f"{split}_data.pkl")

    # Build annotations list in PoseDataset format
    annotations = []
    for i in range(len(all_keypoints)):
        anno = {
            "frame_dir": video_names[i].replace(".mp4", ""),
            "total_frames": NUM_FRAMES,
            "label": int(all_labels[i]),
            "keypoint": all_keypoints[i][
                np.newaxis, ...
            ],  # Add person dimension: (1, 100, 17, 2)
            "keypoint_score": all_keypoint_scores[i][
                np.newaxis, ...
            ],  # Add person dimension: (1, 100, 17)
        }
...
def main():
    ...
    print(f"Train samples: {len(train_data['annotations']) if train_data else 0}")
    print(f"Val samples: {len(val_data['annotations']) if val_data else 0}")
    print(f"\nNext steps:")
    print("1. Create training config: configs/custom_action_recognition.py")
    print(
        "2. Run training: python -m mmaction.train configs/custom_action_recognition.py"
    )
    print("=" * 60)
